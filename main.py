...
def AppendToQueue(url, yt_apikey=None, remove=False):
    ytvideolink = re.compile("^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|live\/|v\/|shorts\/)?)([\w\-]{11})(\S+)?$")
    ytplaylistlink = re.compile("PL[\w\-]{32}")
    ytchannellink = re.compile("^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:channel\/|c\/)?)([\w\-]{24}|\@[\w\-]+)(\S+)?$")
    ...
    if m_video:
        ...
        if remove:
            with open(WATCHLIST, 'r+') as f:
                ...
                if lines:
                    ...
                    f.writelines(lines)
                    f.truncate()
            items = [ExtensionResultItem(icon='images/remove.png',
        ...
        if remove:
            ...
            return RenderResultListAction(items)
        try:
            playlist_info = requests.get(pl_info, params={'id':playlist_id,'part':['snippet','contentDetails'],'key':yt_apikey})
            ...
            os.system(f"wget -O {HERE}/images/{playlist_id}.jpg {thumb} && " +
                      f"convert {HERE}/images/{playlist_id}.jpg {HERE}/images/{playlist_id}.png && " +
                      f"rm {HERE}/images/{playlist_id}.jpg")
            items = [ExtensionResultItem(icon=f'images/{playlist_id}.png',
        ...
        try:
            ...
            os.system(f"wget -O {HERE}/images/{channel_id}.jpg {thumb} && " +
                      f"convert {HERE}/images/{channel_id}.jpg {HERE}/images/{channel_id}.png && " +
                      f"rm {HERE}/images/{channel_id}.jpg")

            items = [ExtensionResultItem(icon=f'images/{channel_id}.png',
...
class ItemEnterEventListener(EventListener):
    ...
    def on_event(self, event, extension):
        ...
        if event.get_data().startswith(append) or event.get_data().startswith(remove):
            return AppendToQueue(event.get_data()[2:],
...
class KeywordQueryEventListener(EventListener):
    ...
    def on_event(self, event, extension):
        ...
        if event.get_argument() and event.get_argument().startswith(append):
            ...
            items.append(ExtensionResultItem(icon='images/remove.png',
                                             name="Remove " + event.get_argument()[2:],
                                             description="I'll check if it's a valid youtube link",
                                             on_enter=ExtensionCustomAction(event.get_argument(),
                                                                            keep_app_open=True)))
        elif event.get_argument() and event.get_argument() == watch:
            items.append(ExtensionResultItem(icon='images/watch.png',
            ...
            try:
                videos_info = requests.get(yt_info, params={'id':ids,'part':parts,'key':yt_apikey},
                                           timeout=5)
                ...
                for video_info in videos_info:
                    ...
                    video_duration = str(parse_duration(video_info['contentDetails']['duration']))
                    video_published = datetime.fromisoformat(video_info['snippet']['publishedAt']).strftime('%H:%M, %b %-d, %y')
                    video_subtitle = [video_channl, video_duration, video_published]
                    items.append(ExtensionResultItem(icon=f'images/{video_channl_id}.png'
